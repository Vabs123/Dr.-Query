...
class PrepareInstances{
	HashMap<String, Classifier> seParameters;
	HashMap<String, Classifier> iParameters;
	HashMap<String, Classifier> ddParameters;

	public Instances prepareSEInstances(DrugGraph dg, Closeness cl){
		ArrayList<Integer> closeness;

		Instances seTrainingSet = new Instances("SideEffect", fvWekaAttributes, 20);
		seTrainingSet.setClassIndex(3);

		Instance seInstance;
		LinkedList<SideEffect> seList = dg.gSideEffect;
		
		for(SideEffect se:seList){
			closeness = cl.getData("SideEffect", se.name);
			seInstance = new DenseInstance(4);
			seInstance.setValue((Attribute)fvWekaAttributes.elementAt(0), closeness.get(0));
 			seInstance.setValue((Attribute)fvWekaAttributes.elementAt(1), closeness.get(1));
 			seInstance.setValue((Attribute)fvWekaAttributes.elementAt(2), closeness.get(2));
 			seInstance.setValue((Attribute)fvWekaAttributes.elementAt(3), "positive");
 			seTrainingSet.add(seInstance);
		}
	}

	public Instances prepareIInstances(DrugGraph dg, Closeness cl, ){

	}

	public Instances prepareDDInstances(DrugGraph dg, Closeness cl, ){

	}

	public void learnSEParameters(){

	}

	public void learnIParameters(DrugGraph dg, Closeness cl){

	}

	public void learnDDParamters(DrugGraph dg, Closeness cl){
